...
public class UpdateAgreementCommandHandler : IRequestHandler<UpdateAgreementCommand, bool>
{
    ...
    public UpdateAgreementCommandHandler(IGenericWriteRepository<Customer, int> customerWriteRepository, IGenericReadRepository<Agreement, int> agreementReadRepository, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<bool> Handle(UpdateAgreementCommand request, CancellationToken cancellationToken)
    {
        ...
        loadedCustomer.UpdateAgreement(request.GetUserId(), request.Id, request.ProductId, DateOnly.Parse(request.AgreementDate), request.DurationInMonths, request.ExtraUsersCount);
